	...
	def __str__(self, index=0, depth=4):
		...
		if isinstance (self._c, list):
			if len (self._c) == 0:
				...
				s.append(f'{indent}matched = true;\n')
			elif len (self._c) == 1:
				...
				if index:
					s.append(f'{self._indent(depth + 2)}buffer->idx + {index + 1} < count &&\n')
				s.extend(
				...
				s.append(f'{indent}{{\n')
				s.extend(
					f'{self._indent(depth + 1)}(void) buffer->next_glyph ();\n'
					for _ in range(index)
				)
				s.extend((f'{self._indent(depth + 1)}matched = true;\n', f'{indent}}}\n'))
		else:
			s.extend(
				(
					f"{indent}switch (buffer->cur ({index or ''}).codepoint)\n",
					f'{indent}{{\n',
				)
			)
			...
			for body, labels in sorted (cases.items (), key=lambda b_ls: sorted (b_ls[1])[0]):
				...
				if len (labels) % 4 != 0:
					s.append ('\n')
				s.extend((body, f'{self._indent(depth + 2)}break;\n'))
			s.append(f'{indent}}}\n')
		return ''.join (s)
